...
typedef enum { Inactivite, Affichage, Clavier, ClavierSpecial, Souris,
	    BoutonSouris, Initialisation, Redimensionnement } EvenementGfx;
typedef enum { GaucheAppuye, GaucheRelache, DroiteAppuye,
	    DroiteRelache } EtatBoutonSouris;
typedef enum { SuiteDePoints, SuiteDeLignes, SuiteDeTriangles,
	    SuiteDeRectangles, BandeDeTriangles,
	    BandeDeRectangles } PrimitiveGfxLib;
typedef enum { ToucheF1 =
	    1, ToucheF2, ToucheF3, ToucheF4, ToucheF5, ToucheF6, ToucheF7,
	    ToucheF8, ToucheF9, ToucheF10, ToucheF11, ToucheF12,
...
void triangle(float xCoin1, float yCoin1, float xCoin2, float yCoin2,
	      float xCoin3, float yCoin3);
...
void ecrisImage(int x, int y, int largeur, int hauteur,
		const unsigned char *donnees);
...
void ecrisImageARVB(int x, int y, int largeur, int hauteur,
		    const int *donneesARVB);
